...
func (p *Parser) Parse(text string) (*AccessLog, error) {
	...
	if accessLog.UriParam != "" {
		decodedURIParam, err := url.QueryUnescape(accessLog.UriParam)
		if err != nil {
			log.Printf("Error decoding URI parameters: %v", err)
		} else {
			accessLog.UriParam = decodedURIParam
		}
	}
